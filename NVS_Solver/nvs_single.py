...
def svd_render_and_postprocess(original_image, model_image, masks, cond_image, image_path, output_path, 
                               lambda_ts, lr, weight_clamp, original_height, original_width, model_height, model_width,
                               K, poses, depth_path=None): 
    """
    执行完整的流程：SVD渲染 -> 保存指定帧对和深度图
    """
    # 1. SVD 渲染 (NVS推理)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_exts', type=list, default=['.png', '.jpg', '.jpeg'], help='需要处理的图片后缀')
    ...
    for img_file in os.listdir(input_folder):
        ...
        # 构造对应深度图路径（input_folder/depth/图片名.npy）
        ...
        print(f"\n=====================================")
        print(f"开始处理图片：{img_file}")
        print(f"=====================================")
        
        # ========== 单张图片的输出路径：基础输出路径/图片名/ ==========
        # 每张图片的结果保存到「基础输出路径/图片名/」下
        ...
        try:
            ...
            print(f"⚠️  跳过 {img_file}：深度图加载失败 - {e}")
            continue
        ...
        try:
            ...
            print(f"⚠️  跳过 {img_file}：图片加载失败 - {e}")
            continue
        ...
        print(f"📌 {img_file} - 图像中心的绝对深度值 Z_center: {Z_center:.4f} 米")
        
        # 自动计算radius和end_position
        radius = Z_center / 2
        end_position = (radius, 0, 0.6)
        print(f"📌 {img_file} - 自动计算的 radius: {radius:.4f} 米")
        print(f"📌 {img_file} - 自动计算的 end_position: {end_position}")
        ...
        try:
            ...
            print(f"⚠️  跳过 {img_file}：生成warped图像失败 - {e}")
            continue
        
        print(f"📌 {img_file} - 原始图像分辨率: {original_width}x{original_height}")
        print(f"📌 {img_file} - 模型输入分辨率: {model_width}x{model_height}")
        ...
        try:
            ...
            print(f"⚠️  {img_file} - Sigma文件未找到，使用默认lambda_ts")
        ...
        try:
            ...
            print(f"⚠️  跳过 {img_file}：SVD渲染失败 - {e}")
            continue
        
        print(f"✅ {img_file} 处理完成！结果保存到: {main_output_path}")
        print(f"   帧对文件夹路径：")
        for suffix in ["0004", "0006", "0008", "0010"]:
            frame_pair_folder = os.path.join(main_output_path, f"{img_name}_{suffix}")
            print(f"   - {frame_pair_folder}")
    
    print(f"\n🎉 所有图片处理完成！总输出根路径：{args.folder_path}")