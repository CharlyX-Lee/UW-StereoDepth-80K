...
def generate_camera_poses(start_position, end_position, num_poses):
    """
    Generate camera poses from the starting point to the end point, 
    ensuring that the camera is always facing the target point.    
    """
    poses = []
    cams=[]
...
def svd_render(image_o,masks, cond_image,image_path,warped_img_path,output_path,lambda_ts,lr,weight_clamp):
    pipe = StableVideoDiffusionPipeline.from_pretrained("stabilityai/stable-video-diffusion-img2vid-xt", torch_dtype=torch.float16, variant="fp16")
    pipe.to("cuda")
    frames = pipe([image_o],temp_cond=cond_image,mask = masks,lambda_ts=lambda_ts,lr=lr,weight_clamp=weight_clamp,num_frames=25, decode_chunk_size=8,num_inference_steps=100).frames[0]
...
if __name__ == "__main__":
    ...
    image_o,masks, cond_image= save_warped_image(save_path,args.image_path,depth_path,num_frames,args.radius,args.end_position)
